...
let gameState = {
    ...
    winnerName: '',
    players: {
        1: { id: null, name: '', hp: 200, maxHp: 200, shield: 0, gold: 0, tier: 1, cooldownStart: 0, cooldownUntil: 0, cooldownDuration: 0, blockedUntil: 0, machineGuns: 0 },
        2: { id: null, name: '', hp: 200, maxHp: 200, shield: 0, gold: 0, tier: 1, cooldownStart: 0, cooldownUntil: 0, cooldownDuration: 0, blockedUntil: 0, machineGuns: 0 }
    },
    entities: { projectiles: [], minions: [] }
};
...
function resetGame() {
    ...
    [1, 2].forEach(slot => {
        ...
        gameState.players[slot].cooldownStart = 0;
        gameState.players[slot].cooldownUntil = 0;
        gameState.players[slot].cooldownDuration = 0;
...
function applyDamage(slot, amount, ignoreShield = false, isNuke = false) {
    ...
    if (p.hp <= 0 && gameState.status === 'playing') {
        ...
        gameState.winnerName = gameState.players[slot === 1 ? 2 : 1].name || (slot === 1 ? '2P' : '1P');
...
setInterval(() => {
    ...
    gameState.entities.minions.forEach(m => {
        ...
        if (Math.abs(m.x - targetX) > 1) {
            m.x += (m.owner === 1 ? 1 : -1) * 50 * dt;
            m.state = 'walk';
        } else {
            m.state = 'attack'; // 進入攻擊動畫狀態
            m.lastAttack += dt;
            if (m.lastAttack >= 1.5) {
                m.lastAttack = 0;
    ...
    for (let i = gameState.entities.projectiles.length - 1; i >= 0; i--) {
        ...
        if (p.type === 'knife') {
            for (let j = 0; j < gameState.entities.minions.length; j++) {
                ...
                if (m.owner === targetSlot) {
                    let crossed = p.owner === 1 ? (p.x >= m.x) : (p.x <= m.x);
                    if (crossed) {
                        gameState.entities.minions.splice(j, 1);
                        hit = true;
                        break;
                    }
                }
        ...
        if (!hit && reachedCastle) {
            ...
            if (p.type === 'knife') applyDamage(targetSlot, 1);
                ...
                gameState.entities.minions = gameState.entities.minions.filter(m => m.owner === p.owner);
            }
        }

        if (hit) gameState.entities.projectiles.splice(i, 1);
    }
...
io.on('connection', (socket) => {
    ...
    socket.on('hitBlock', () => {
        ...
        const duration = (p.tier === 3) ? 1000 : 3000;
        p.cooldownStart = now;
        p.cooldownUntil = now + duration;
        p.cooldownDuration = duration;
        ...
        if (selected.type === 'gold') {
            p.gold += selected.val;
            io.emit('goldEffect', { slot: slot, amount: selected.val }); // 發送給所有玩家動態金幣特效
        }
        ...
        if (selected.type === 'heal') p.hp = Math.min(p.maxHp, p.hp + (p.maxHp * selected.val));
        ...
        if (selected.type === 'summon') {
            ...
            for (let i = 0; i < selected.val; i++) {
                setTimeout(() => {
                    gameState.entities.minions.push({ id: nextEntityId++, owner: slot, x: slot === 1 ? 0 : 150, lastAttack: 0, state: 'walk' });
                }, i * interval);
            }
        }
    });
...
server.listen(PORT, () => {
    console.log(`後端伺服器順利運作中: http://localhost:${PORT}`);
});
setInterval(() => {
    if (gameState.status !== 'playing') return;
    const dt = 1 / 30;
